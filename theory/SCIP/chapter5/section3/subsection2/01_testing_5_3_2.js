...
function make_execution_function(inst, labels, machine, pc, flag, stack, ops) {
    ...
    return x === "assign"
           ...
           ? () => {
               display(get_register_contents(machine, "the_heads"));
               display(get_register_contents(machine, "the_tails"));
               advance_pc(pc);
           }
...
function make_test(inst, machine, labels, operations, flag, pc) {
    ...
    const condition_fun = make_operation_exp(condition, machine,
                                             labels, operations);
